        ...
        <div className="space-y-3 font-mono text-caption">
            ...
            <div className="grid grid-cols-3 gap-1">
              {(['sgd', 'momentum', 'adam'] as const).map((o) => (
                <button
                  ...
                  className={
                    'text-label uppercase tracking-[0.12em] font-mono py-1 rounded border focus-ring transition-colors ' +
                    (optimizer === o
          ...
          <div className="pt-3 border-t border-border space-y-1">
            <button
              ...
              onClick={onTrain}
              disabled={!train}
              className="focus-ring w-full text-label uppercase tracking-[0.12em] font-mono py-2 rounded border border-accent text-accent hover:bg-accent-soft transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
...
function PresetButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="focus-ring text-label uppercase tracking-[0.12em] font-mono px-2 py-0.5 rounded border border-border text-muted hover:text-ink hover:border-accent transition-colors"
    >
      {labels[id]}
    </button>
  );
...
function LossChart({
  ...
  if (losses.length === 0) {
    return (
      <div
        className="rounded border border-border bg-bg/40 flex items-center justify-center font-mono text-label text-dim"
  ...
  if (finite.length === 0) {
    return (
      <div
        className="rounded border border-border bg-bg/40 flex items-center justify-center font-mono text-label text-[rgb(var(--negative))]"
        style={{ width: '100%', height }}
      >
        Loss diverged
      </div>
